...
|---|---|---|
| Open **Releases**, choose the newest version and select **vesta-companion.zip** under **Assets**. Then choose **Extract all**. Keep the files together in a private folder. | Select the folder as a **local project** in your approved agent, or connect it as a local folder. | Send the message below. Your Companion will guide you through setup. |
...
**[Try the interactive demo](https://vesta-software-group.github.io/vesta-companion/demo.html)** with fictional tasks. The ZIP includes an offline copy at **examples/board.html**. Your own board starts empty.
...
Your profile, notes, tasks, projects and custom skills live under **personal/**. Setup creates a ready-to-use memory vault with a HOME page, inbox, knowledge, contacts, decisions, meetings, journal and archive folders. Notes have clear dates, sources and confidence labels. Open **personal/vault** in Obsidian if you want a visual notebook; no community plugins are needed.
...
[PDF user guide](docs/Quick-start.pdf) · [Security and private reporting](SECURITY.md) · [Core skills](skills/INDEX.md) · [Compatibility](docs/COMPATIBILITY.md) · [Test evidence](docs/TESTING.md) · [Contributing](CONTRIBUTING.md)
...
For an extra confirmation on Claude shell commands and file edits, ask about the [optional approval settings](docs/CLAUDE-GUARDRAILS.md). They are opt-in and preserve your organisation's controls.
