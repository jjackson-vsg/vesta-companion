...
5. Use scripts/companion.py init if Python is already available; it is additive and preserves existing files. Otherwise use templates/FILE-OPERATIONS.md. Do not require a user terminal or silently install software.
6. Show the person their personal folder and explain local memory, the app's cloud processing, approved-account use and the need for a private location. Offer Obsidian as an optional way to browse personal/vault. Do not install plugins.
