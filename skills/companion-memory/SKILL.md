...
1. Read the profile and handover, then search personal/vault by HOME.md, filenames, metadata and content. Follow direct source links as needed. Distinguish actual evidence from inference; do not invent missing notes.
2. Save new durable knowledge in a clearly named Markdown file under personal/vault. Use templates/note.md. Include date, type, source and confidence where relevant. Never save credentials, token-bearing links or one-time codes.
...
4. Keep contacts in vault/contacts, decisions in vault/decisions and reference knowledge in vault/knowledge. Link the note from HOME.md. Tasks belong in todo/tasks.json, not duplicate tick lists.
